...
- Git branch，默认 `main`
- Git operation 总预算，默认 `30` 秒
- push 总尝试次数，默认 `3`（首次尝试加最多 2 次重试）
...
- 单实例检查通过后，创建一个无窗口的启动同步工作线程；本次同步共享 30 秒总 operation deadline，完成或超时前不调用 `load_config()`、不创建 `TokenWidget`。
- 主线程通过 Qt 事件循环等待工作线程信号，因此 Windows 不会将进程判断为无响应；本功能不新增启动等待界面。
- 退出同步在工作线程执行，本次同步同样共享 30 秒总 operation deadline。
- 退出请求后立即禁用窗口、停止刷新 timer，并阻断设置、导入、刷新及 worker 写回；同时禁止重复退出，避免数据库在 push 读取后再次变化。
- 每个 Git 子进程使用 operation deadline 的剩余预算；超时由 `subprocess.run` 终止并等待。SQLite、文件写入和 `fsync` 等本地调用在阶段完成后检查预算，不承诺抢占正在执行的标准库系统调用；预算耗尽后不进入下一阶段，并按失败降级策略继续启动或退出。
...
- 推送竞争：最多进行 3 次 push 尝试；每次竞争后都从最新远端 tree 重建，且所有尝试共享同一 operation deadline。
