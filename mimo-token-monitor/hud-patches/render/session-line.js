...
export function renderSessionLine(ctx) {
    ...
    if (display?.showUsage !== false && ctx.usageData && !shouldHideUsage(ctx.stdin)) {
        ...
        if (ctx.usageData.balanceLabel) {
            const percent = ctx.usageData.fiveHour ?? ctx.usageData.sevenDay;
            if (percent !== null) {
                const usageBarEnabled = display?.usageBarEnabled ?? true;
                const bar = usageBarEnabled ? `${quotaBar(percent, barWidth, colors)} ` : '';
                parts.push(`${label(t('label.usage'), colors)} ${bar}${ctx.usageData.balanceLabel}`);
            }
            else {
