...
- **用量总览页面**：标题栏切换到“总览”后，以 9pt 单行文字显示 Token Plan 剩余天数、WLB 左侧 `WLB`/右侧周重置剩余天数，以及 GPT 左侧主窗口重置时分/右侧周重置剩余天数；各栏百分比右对齐并配左右分段进度条
...
- 切换后悬浮窗显示剩余百分比、已用百分比、7 天窗口和状态；接口返回 `reset_at` 时显示重置日期时间，总览中的 WLB 右侧只显示周重置剩余天数（如 `5天`，缺失时为 `--天`）
